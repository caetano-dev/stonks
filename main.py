...
mensagem_bonitinha = (
    'Analisando minha bolsa de valores:chart_with_upwards_trend:...Por favor aguarde.')
...
async def help(ctx):
    ...
    embed.add_field(
        name=".dolar", value='Informa a cotação do Dólar.', inline=False)
    embed.add_field(
        name=".franco", value='Informa a cotação do Franco Suíço.', inline=False)
    embed.add_field(name=".canadense", value='Informa a cotação do Dólar Canadense.',
                    inline=False)
    embed.add_field(
        name=".iene", value='Informa a cotação do Iene.', inline=False)
    embed.add_field(
        name=".euro", value='Informa a cotação do Euro.', inline=False)
    embed.add_field(
        name=".rublo", value='Informa a cotação do Rublo.', inline=False)
    embed.add_field(
        name=".honkong", value='Informa a cotação do Dólar de Hong Kong.', inline=False)
    embed.add_field(
        name=".mexico", value='Informa a cotação do Peso mexicano de Hong Kong.', inline=False)
    embed.add_field(
        name=".coroa", value='Informa a cotação da Coroa dinamarquesa de Hong Kong.', inline=False)
    embed.add_field(name=".converter",
                    value='Converte um valor em Dólar para Real.', inline=False)
    embed.add_field(
        name=".vbuck", value='Informa o preço do Vbuck.', inline=False)
    embed.add_field(name=".roubo", value='Fala umas verdades.', inline=False)
    embed.add_field(
        name=".video", value='Sugere um video do youtube sobre stonks.', inline=False)
...
async def vbuck(ctx):
    ...
    sleep(1)
    await ctx.send('KKKKKKKKK VBUCK TA CARO DEMAIS MANO. VAI DAR NAO.')
...
async def converter(ctx, arg=1):
    try:
        ...
        cotacao = json.loads(requisicao.text)
        valor = cotacao["rates"]["BRL"]
        multiplicacao = int(arg) * float(valor)
...
async def video(ctx):
    try:
        ...
        for link in soup.find_all('a'):
            links = (link.get('href'))
            if "/watch?v=" in links and "https" not in links and "radio=" not in links:
                videos.append(links)
        x = randint(0, 19)
